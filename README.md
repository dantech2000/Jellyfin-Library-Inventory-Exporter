...
```text
https://dantech2000.github.io/Jellyfin-Library-Inventory-Exporter/manifest.json
```
...
When the release is published, the manifest workflow writes `manifest.json` to the `gh-pages` branch. GitHub Pages must be enabled for the repository with `gh-pages` as the source branch so Jellyfin can read the manifest URL above.
